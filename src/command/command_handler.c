...
void command_args_handler(void *arg_table, char program_name[], struct config *_config, struct command_args *_args) {
    ...
    _config->execute_path = (char *) _args->exec_path->sval[0];
    if (_args->input_path->count > 0) {
        _config->input_path = (char *) _args->input_path->sval[0];
    }
    if (_args->output_path->count > 0) {
        _config->output_path = (char *) _args->output_path->sval[0];
    }
    if (_args->err_path->count > 0) {
        _config->error_path = (char *) _args->err_path->sval[0];
    }
    ...
    if (_args->args->count > 0) {
        for (; i < _args->args->count + 1; ++i) {
            _config->args[i] = (char *) _args->args->sval[i - 1];
        }
    }
    _config->args[i] = NULL;
    i = 0;
    if (_args->env->count > 0) {
        for (; i < _args->env->count; ++i) {
            _config->env[i] = (char *) _args->env->sval[i];
        }
    }
    _config->env[i] = NULL;
}
